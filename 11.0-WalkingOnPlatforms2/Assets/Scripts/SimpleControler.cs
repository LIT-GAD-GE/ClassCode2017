...
public class SimpleControler : MonoBehaviour
{
	...
	public void Move(float hAxisValue, float vAxisValue, bool doJump, bool doDuck, bool doClimb) {
		...
		if (grounded || canMoveInAir) {
			...
			if (grounded) {
				// If the player is pressing the duck button i.e. doDuck is false, but we were previously 
				// ducking, i.e. anim.GetBool ("Crouch") is true, and there is a platform overhead THEN
				// set doDuck to true (enev though the player isn't pressing the duck button) as there is
				// a platform above us and as such we shouldn't stand up
